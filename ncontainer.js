let addButton = document.getElementById("add_a_container"); // Button for adding a container
...
let inputName = document.getElementById("str-inp-name"); // String input
let containerName = ''; // Let for keeping the container's name input
let inputDescription = document.getElementById("str-inp-info"); // String input
let containerDescription = ''; // Let for keeping the container's description input
let baseList = document.getElementById("available-containers"); // List of the containers
let close = document.getElementById("cross1"); // Th modal window's cross
        ...
        if (xhr.readyState == 4) { // The answer has been got
            if(xhr.status == 200) { // The server's returned code 200 (success)
                ...
                if (results[0] === "1") { // If the inserting into database was successful insert into the list
                    ...
                    tmp.innerHTML = "&#9773; " + containerName; // Inserting text
                    ...
                    baseList.appendChild(tmp) // Appending to the ul element
                }
                else {
